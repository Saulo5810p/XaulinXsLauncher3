#!/usr/bin/env python3
"""
Aplica os 4 fixes da sessao do XaulinXsLauncher3, cada um de forma
INDEPENDENTE (nao e tudo-ou-nada como a v1) -- roda quantas vezes
precisar, aplica so o que ainda falta em cada arquivo, mesmo que
alguns fixes ja tenham sido aplicados em sessoes anteriores.

1) FONTE CUSTOM QUEBRANDO WIDGETS (XaulinXsGlobalFontInflaterFactory.kt)
   Launcher.onCreate() passa "this" (a propria Activity) como Context
   para LauncherWidgetHolder/LauncherAppWidgetHost, entao
   ...
   foi instalado. RemoteViews.apply() ja instala seu proprio
   LayoutInflater.Filter nesse inflater compartilhado para sandboxing
   -- o Factory2 rodando no meio disso quebrava a inflacao de
   QUALQUER widget. Fix: o Factory2 detecta se esta dentro de um
   AppWidgetHostView e devolve null imediatamente nesse caso.

2) QSB INVISIVEL EM DENSIDADE REAL (HotseatProfile.kt)
   Hotseat.onMeasure() mede a QSB com MeasureSpec.EXACTLY usando
   HotseatProfile.qsbWidth, que tinha um branch
   "if (!isScalableGrid) return ...qsbWidth=0...". Grids de TELEFONE
   comuns (ex. "5_by_5"/Large Phone, o grid do Galaxy A35) nao
   declaram isScalable em device_profiles.xml -- default false --
   qsbWidth sempre zero, QSB nunca aparece. Fix: calcula qsbWidth de
   verdade tambem nesse branch, sem ativar o algoritmo de scaling de
   grid escalavel.

3) QSB LIGADA AO WIDGET DE BUSCA DO GOOGLE (OseWidgetManager.kt)
   handleOseInfoUpdate() localizava e bindava um AppWidget REAL de
   terceiro (RemoteViews) do motor de busca padrao do sistema -- no
   caso comum (Google como app padrao), isso injetava o widget real
   de busca do Google na QSB. Consequencias: (a) long-press na QSB
   abria as CONFIGURACOES REAIS DO APP GOOGLE (a tela "Personalizar"
   com tema/transparencia/atalho — nao e algo nosso pra reproduzir,
   e o proprio widget de terceiro vazando pra dentro do nosso
   launcher); (b) a QSB ficava sujeita ao mesmo problema do item 1.
   Fix: handleOseInfoUpdate() nunca mais localiza/binda um AppWidget
   de terceiro -- sempre despacha valores nulos, o framework cai
   automaticamente em OseWidgetView.getErrorView(), a UI estatica
   nativa do AOSP, sem RemoteViews de terceiro e sem config no
   long-press.
...
4) TITULOS SEM FONTE CUSTOMIZADA (XaulinXsCustomFont.kt +
   ...
   XML); o header "XaulinXs Customizations" e inflado por um
   LayoutInflater CLONADO que nem sempre carrega o Factory2 global.
   Fix: applyToSettingsTitle() e applyToPreferenceListRecycling().
...
Cada fix e independente -- se 2 ja estao aplicados e 2 nao, ele aplica
so os 2 que faltam, sem travar (diferente da v1 do script).
"""
import argparse
import subprocess
import sys
...
_fix(
    ...
    patch_filename="fix_4_titles.patch",
    patch_content=r'''
...
def main():
    ...
    for fix in FIXES:
        ...
        if fix["marker"] in target.read_text(encoding="utf-8"):
    ...
    if already_applied:
        ...
        for p in already_applied:
            print(f"  = {p}")
    if failed:
        print("NAO aplicados (nao bateram limpo contra o estado atual do arquivo):")
        for p, err in failed:
            print(f"  ! {p}")
            print(f"    {err.splitlines()[-1] if err else '(sem detalhe)'}")
        print(
            "\nPara os itens marcados com '!', o arquivo local ja foi editado de "
            "forma diferente do esperado. Descreva o que mudou nesse arquivo e "
            "eu gero um patch novo em cima do estado atual dele."
        )
    ...
    print(
        ...
        "terceiro (nem do widget do Google)\n"
